...
public class GameCanvas : IActionListener
{
			...
			if (Controller.isConnectionFail)
			{
				if (!Controller.isMain)
				{
					if (currentScreen == serverScreen && ServerListScreen.isGetData && !Service.reciveFromMainSession)
					{
						...
						serverScreen.cancel();
						GD.Print("connect fail 1");
					}
					if (currentScreen == loginScr && !Service.reciveFromMainSession)
					{
						onConnectionFail();
						GD.Print("connect fail 2");
					}
				}
				else
				{
	...
	{
		if (InfoDlg.isShow || (msgdlg != null && msgdlg.info != null && msgdlg.info.Equals(mResources.PLEASEWAIT)) || Char.isLoadingMap || LoginScr.isContinueToLogin)
		{
			return true;
		}
		return false;
	...
	public void paint(mGraphics gx)
	{
		try
		{
			// Copy drawContext from gx to our internal g so drawing actually works
			g.SetDrawContext(gx.GetDrawContext());
