...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public void show2()
	{
		GD.PushWarning(">>>>ServerListScreen show2: ");
		GameScr.cmx = 0;
		GameScr.cmy = 0;
		initCommand();
		loadScreen = false;
	...
	{
		GD.PushWarning(">>>>SetIpSelect: " + index + "  save:" + issave);
		ipSelect = index;
		if (issave)
		{
			Rms.saveRMSInt(RMS_svselect, ipSelect);
	...
	public void Login_New()
	{
		...
		if (!flag && !flag2)
		{
			if (text == null || text.Equals(string.Empty))
			{
				GD.PushWarning(">>>>Login_New: login2: ");
				Service.gI().login2(string.Empty);
			}
			else
			{
	...
	public static void CheckBack_ServerListScreen()
	{
		...
		GD.PushWarning(">>>>CheckBack_ServerListScreen: " + ipSelect + "  auto login:" + isAutoLogin);
