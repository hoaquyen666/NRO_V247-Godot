...
public class Session_ME : ISession
{
	...
	private class MessageCollector
	{
		public void run()
		{
			...
			{
				GD.Print("error read message!");
				GD.Print(ex3.Message.ToString());
			}
			if (!connected)
			{
				return;
		...
		private Message readMessage()
		{
			...
			{
				GD.Print(ex.StackTrace.ToString());
			}
			return null;
		}
	}
	...
	public void connect(string host, int port)
	{
		...
		{
			GD.Print(">>>return connect ...!" + connected + "  ::  " + connecting);
			return;
		}
		if (mSystem.currentTimeMillis() < timeWaitConnect)
		{
			GD.PushWarning(">>>>chặn việc nó kết nối 2 3 lần liên tục");
			return;
		}
