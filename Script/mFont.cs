...
public class mFont
{
	...
	public Godot.Font myFont;

	private int height;

	private int wO;

	public Godot.Color color1 = Colors.White;

	public Godot.Color color2 = Colors.Gray;
	...
	{
		string fontName = "Arial"; // Default fallback font
		int fontSize = 14;
		
		if ((id > 0 && id < 10) || id == 19)
		{
			yAdd = 1;
			fontName = "Arial"; // barmeneb equivalent
			fontSize = 12;
		}
		else if (id >= 10 && id <= 18)
		{
			fontName = "Arial"; // chelthm equivalent
			yAdd = 2;
			fontSize = 14;
		}
		else if (id > 24)
		{
			fontName = "Arial"; // staccato equivalent - larger for big numbers
			fontSize = 18;
		}
		...
		// Create SystemFont instead of loading .tres file
		try
		{
			var systemFont = new SystemFont();
			systemFont.FontNames = new string[] { fontName, "Segoe UI", "Tahoma", "Sans-Serif" };
			systemFont.Antialiasing = TextServer.FontAntialiasing.Lcd;
			myFont = systemFont;
		}
		catch (System.Exception ex)
		{
			Cout.LogError("Failed to create SystemFont: " + ex.Message);
			myFont = null;
		}
	...
	public Godot.Color setColor(int rgb)
	{
		int num = rgb & 0xFF;
		int num2 = (rgb >> 8) & 0xFF;
	...
	public Godot.Color bigColor(int id)
	{
		Godot.Color[] array = new Godot.Color[7]
		{
			Colors.Red,
			Colors.Yellow,
			Colors.Green,
	...
	public Godot.Color setColorFont(sbyte id)
	{
		...
		if (mGraphics.zoomLevel == 1)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				if (num2 > -1)
				{
					...
					if (num3 + num4 > imgFont.texture.GetHeight())
					{
						num3 -= imgFont.texture.GetHeight();
						x2 = imgFont.texture.GetWidth() / 2;
					}
	...
	public int getWidthExactOf(string s)
	{
		try
		{
			if (myFont != null)
			{
				return (int)myFont.GetStringSize(s).X / mGraphics.zoomLevel;
			}
	...
	public int getHeight()
	{
		...
		try
		{
			if (myFont != null)
			{
				height = (int)myFont.GetStringSize("Adg").Y + 2;
			}
			else
			{
				height = 20;
	...
	{
		y0 += yAddFont;
		int textAlign = 0; // LEFT by default
		float xPos = x0;
		float yPos = y0;
		
		// Godot's DrawString uses width and alignment together
		// For proper alignment, we need to pass the original x position
		// and let Godot handle alignment based on width parameter
		switch (align)
		{
		case 0: // LEFT
			textAlign = 0;
			break;
		case 1: // RIGHT
			// For right align, x0 is the right edge position
			// We need to calculate left position by subtracting text width
			if (myFont != null)
			{
				float textWidth = myFont.GetStringSize(st).X;
				xPos = x0 - textWidth / mGraphics.zoomLevel;
			}
			textAlign = 0; // Draw as left-aligned at calculated position
			break;
		case 2: // CENTER
		case 3:
			// For center align, x0 is the center position
			// Calculate left position by subtracting half of text width
			if (myFont != null)
			{
				float textWidth = myFont.GetStringSize(st).X;
				xPos = x0 - textWidth / 2 / mGraphics.zoomLevel;
			}
			textAlign = 0; // Draw as left-aligned at calculated position
			break;
		}
		g.drawStringWithColor(st, (int)xPos, (int)yPos, color1, myFont, textAlign);
	}
